...
void ShaderProgram::load(const char *vertex_shader_file, const char *fragment_shader_file) {
    ...
    m_light_pos_uniform = glGetUniformLocation(m_program_id, "lightPosition");
...
void ShaderProgram::set_light_pos(glm::vec3 position)
{
    glUseProgram(m_program_id);
    glUniform2f(m_light_pos_uniform, position.x, position.y);
}
